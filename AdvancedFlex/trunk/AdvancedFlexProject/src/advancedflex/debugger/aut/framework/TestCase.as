...
package advancedflex.debugger.aut.framework {
	...
	public class TestCase implements advancedflex.debugger.aut.framework.Test {
		...
		public function getTestHeader(isHead:Boolean = true):XML {
			setUpTestsSync();
			var result:XML = <TestCase name={getQualifiedClassName(this)}/>;
			if(isHead) {
				result = XML(
					<aut-list>
					</aut-list>
			...
			for each(item in testSet) {
				result.appendChild(<method name={item.@name} type="test" />);
			}
			for each(item in timeSet) {
				result.appendChild(<method name={item.@name} type="time" />);
			}
			return result;
		}
		 ...
		 */
		public function runTest(out:AUTOutput, options:String = ""):void {
			this.out = out;
			setUpTestsSync();
			testSync();
		}
