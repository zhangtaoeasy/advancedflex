...
package advancedflex.graphics.images.imagefilters {
	 ...
	 * 
	 * @see advancedflex.graphics.images.imagefilters.IImageFilter
	 */
	public class AbstractImageFilter implements IImageFilter {
			...
			):void
		{
			...
			if(inputBitmapData != outputBitmapData) {
				outputBitmapData.copyPixels(
					inputBitmapData, sourceRect, destPoint);
				sourceRect.x = destPoint.x;
				sourceRect.y = destPoint.y;
			}
		}
		
		protected function validateApplyParam(
			inputBitmapData:BitmapData, 
			outputBitmapData:BitmapData, 
			sourceRect:Rectangle, 
			destPoint:Point
			):void
		{
			...
			if(!sourceRect){
				throw new ArgumentError("Param:<sourceRect> must not be null.");
			}
			if(!destPoint){
				throw new ArgumentError("Param:<destPoint> must not be null.");
			}
			sourceRect = sourceRect.intersection(inputBitmapData.rect);
		}
		...
		public function clone():IImageFilter {
			throw new IllegalOperationError("Abstract method calling.");
		}
	}
}