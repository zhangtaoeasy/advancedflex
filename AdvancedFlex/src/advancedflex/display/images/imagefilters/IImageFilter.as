...
package advancedflex.display.images.imagefilters {
	...
	public interface IImageFilter {
		 ...
		 * @param destPoint 目标图像（当前 BitmapData 实例）中与源矩形的左上角对应的点。当 inputBitmapData 与 outputBitmapData 相同时忽略此参数。
		 * 
		 * @throws TypeError inputBitmapData、outputBitmapData、sourceRect、destPoint 为空。
		 * @throws flash.errors.IllegalOperationError BitmapData 对象的透明度与滤镜操作不兼容。
		 * 
		...
		/**
		 * 已知 BitmapData 对象、源矩形和滤镜对象，确定 apply() 方法调用所影响的目标矩形。
		 * @param sourceBitmapData BitmapData 对象。
		 * @param rect 一个矩形，它定义要用作输入的源图像的区域。默认为<code>sourceBitmapData.rect</code>。
		 * 
		...
		/**
		 * 返回一个新的 IImageFilter 对象，它是对原始实例的克隆，包含与原始实例完全相同的副本。
		 * 
